...
function Result.FromString(target)
    ...
    if hours == nil then hours, minutes, seconds = target:match("^(%d+):(%d%d):(%d%d%..*)$") end
...
function Result.FromSeconds(target) return Result.FromTicks(target * 60) end
function Result.FromMinutes(target) return Result.FromSeconds(target * 60) end
function Result.FromHours(target) return Result.FromHours(target * 60) end
function Result.FromDays(target) return Result.FromHours(target * 24) end
...
function Result:getTicks()
    return math.floor(
        (((self.Days * 24 + self.Hours) * 60 + self.Minutes) * 60 + math.floor(self.Seconds + 0.5))
            * 60
    )
end
...
function Result:getTimeAsHHMMSS()
    return string.sub(
        tostring(1000000 + self.Hours * 10000 + self.Minutes * 100 + math.floor(self.Seconds)), 2
    )
end
